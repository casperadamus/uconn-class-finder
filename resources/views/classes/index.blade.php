...
<body class="bg-gray-100">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($classes as $index => $class)
                                <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $class['total'] ?? 'N/A' }} seats
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                   <!-- maybe add pagination or limit info here -->
                </div>
            </div>
        ...
        @else
            <div class="bg-white rounded-lg shadow-md p-6 text-center">s
        ...
        <details class="mt-6 bg-white rounded-lg shadow-md p-6">
            <summary class="cursor-pointer text-blue-600 hover:text-blue-800 text-lg font-semibold">Debug Information</summary>
            <div class="mt-4">
                <h4 class="font-semibold mb-2">Current Search Parameters:</h4>
                <ul class="list-disc ml-5 mb-4">
                    <li>Campus: {{ $campus }}</li>
                    <li>Subject: {{ $subject ?? 'All subjects' }}</li>
                    <li>Course Number: {{ $catalogNumber ?? 'Not specified' }}</li>
                    <li>Keyword: {{ $keyword ?? 'Not specified' }}</li>
                    <li>Current Term: 1263 (Spring 2026)</li>
                </ul>
                
                <h4 class="font-semibold mb-2">Sample Class Data (first 2):</h4>
                <pre class="bg-gray-800 text-green-400 p-4 rounded-lg overflow-auto max-h-96"><code>{{ json_encode(array_slice($classes, 0, 2), JSON_PRETTY_PRINT) }}</code></pre>
            </div>
        </details>
    </div>
</body>
